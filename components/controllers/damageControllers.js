...
exports.finishJob = async (req, res) => {
    ...
    if(findJob.length > 0) {
        await dbQuery.execute('UPDATE Bike_repair SET bp_count_use = bp_count_use + 1 WHERE bp_id = ?', [bp_id])
        await dbQuery.execute('UPDATE Damage SET dm_status = 3 WHERE bp_id = ? AND id = ?', [findJob[0].bp_id, findJob[0].id])
        .then(async (response) => {
            await dbQuery.execute('INSERT INTO Notifications (user_id, noti_title, noti_description) VALUES (?, ?, ?)',
            [findJob[0].user_id, "รายการแจ้งเตือนสำเร็จงาน", `รายการที่คุณแจ้งไปยัง ${findJob[0].bp_name} สำเร็จงานเรียบร้อยแล้ว`])
            return res.status(200).send({ msg: 'คุณได้ทำการยืนยันรายนี้สำเร็จแล้ว' })
        })
        .catch((err) => {
            console.log(err)
